...
static char *cmd_type[] = {"M_ONLINE", "M_OFFLINE", "M_GETINFO", "M_SYNCDIR", "M_SYNCFILE", "M_CONFUPDA", "M_SETDIRTIME", "M_GETDIRTIME", "M_DELFILE", "M_EXECUTE", "M_TASKMOD"};
...
int voss_task_mod(StringPairList *pairlist, char *buf, int len)
{
	char *p;
	int i = 0;
	int n = 0;
    ...
    for (i = 0; i < pairlist->iLast; i++ )
	{
		...
		if (mod_task_level(key, atoi(val)))
		{
			if (ol < len)
				ol += snprintf(buf + ol, len - ol, "del %s %s err %m", key, val);
		}
...
void do_request(int fd, int datalen, char *data)
{
	...
	switch(type)
	{
		...
		case M_TASKMOD:
			sendlen = voss_task_mod(pairlist, sendbuf, sizeof(sendbuf));
			if (sendlen == 0)
				sendlen = snprintf(sendbuf, sizeof(sendbuf), "del file OK!");
			break;

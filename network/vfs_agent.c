...
void get_data_from_task(t_vfs_tasklist *task)
{
	...
	size_t n = snprintf(buf, sizeof(buf), "%s|%s|%ld|%ld|%d|%ld|%d\n", base->filename, base->filemd5, time(NULL), base->fsize, base->overstatus, sub->lastlen, base->type);
...
static int do_hb(int fd, int *havedata)
{
	...
	*havedata |= 1;
	LOG(vfs_agent_log, LOG_DEBUG, "do_hb %s:%s:%d\n", ID, FUNC, LN);
	return 0;
}
...
static void do_run(struct threadstat *thst)
{
	...
	int fd = -1;
	char *serverip = NULL;
	int serverport = myconfig_get_intval("voss_server_port", 8001);
	...
	while (!stop)
	{
		...
		if (outdata != 2)
		{
			...
			//send hb
			LOG(vfs_agent_log, LOG_DEBUG, "try do_hb %ld %ld %d\n", now, last, heartbeat);
